...
const ChatBox = ({ username }) => {
  ...
  useEffect(() => {
    ...
    socket.on('user-joined', (data) => {
      if (data.username !== username) {
        setMessages((prev) => [...prev, { sender: 'System', content: `${data.username} has joined the chat.` }]);
      }
      setActiveUsers(data.activeUsers);
    });

    socket.on('user-left', (data) => {
      setMessages((prev) => [...prev, { sender: 'System', content: `${data.username} has left the chat.` }]);
      setActiveUsers(data.activeUsers);
    });
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      socket.emit('message', { sender: username, content: `📎 File shared: ${fileLink}` });
      ...
      <div className="messages h-64 overflow-y-auto border p-4 bg-white rounded">
        {messages.map((msg, index) => (
          <p key={index}>
            ...
            {msg.content.includes('http://localhost:5000/api/chat/download/') ? (
              <a href={msg.content.split('File shared: ')[1]} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">
                Download File
              </a>
            ) : (
              msg.content
